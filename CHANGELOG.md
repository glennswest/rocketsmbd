...
- **feat(#29):** **Pluggable crypto backend — optional OpenSSL for FIPS.** The FIPS-able SMB2/3 primitives (HMAC-SHA256, SHA-512, AES-CMAC, AES-GCM/CCM) now go through a cfg-selected backend: `backend-rustcrypto` (default, pure-Rust, static-musl) or `backend-openssl` (system OpenSSL/libcrypto, dynamically linked, for FIPS deployments). The public `crypto` API is unchanged; NTLM's MD4/MD5/RC4 stay RustCrypto (FIPS can't provide them; a FIPS build uses `--no-default-features`). **Verified on a Linux host (dev.g8.lo):** the OpenSSL backend passes every crypto known-answer test — incl. the RFC 4493 AES-CMAC and AEAD roundtrips — proving identical output to RustCrypto; full matrix clean (openssl±ntlm±kerberos), binary links `libcrypto.so.3`. OpenSSL's one-shot AEAD can't do AES-CCM's length prefix, so CCM (rarely negotiated) stays on the `ccm` crate while GCM (the SMB3 default) uses OpenSSL.
...
- **test(#31,#37):** **Kerberos validated end-to-end against a live KDC.** Stood up realm `G8.LO` (krb5.g8.lo KDC), created `cifs/dev.g8.lo@G8.LO` + keytab, and ran a cifs.ko `sec=krb5` mount against rocketsmbd (`auth = "kerberos"`) on dev.g8.lo: the GSS acceptor authenticated `alice@G8.LO`, derived the SMB session key from the Kerberos sub-session key, and served **md5-verified** read/write. A second run with `require_signing = true` + `sec=krb5,seal` passed too — **SMB3 signing (enforced) and AES-GCM sealing both work over the Kerberos-derived key** (32 MiB sealed I/O, md5 matched). Runbook + `bench/krb5/e2e.sh` in `docs/KERBEROS.md`.
